...
export const PrintLayout = forwardRef<HTMLDivElement, Props>(
  ({ records, globalMode, cols = 1, perPage = 10 }, ref) => {
    // Split records into pages, then each page into rows of `cols`
    ...
    return (
      <div ref={ref} style={{ background: '#fff', width: '100%' }}>
        {pages.map((pageRecords, pi) => {
          // Split page records into rows of `cols`
...
function BarcodeItem({ imgUrl, ascii, type: _type }: { imgUrl: string; ascii: string; type?: string }) {
  return (
