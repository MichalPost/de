...
import { CustomerCodeInput } from '@chemtools/shared/ui/CustomerCodeInput'
import { buildLongCode } from '@chemtools/shared/lib/reagent-code'
import { createBarcodeSvg } from '@chemtools/shared/lib/code128'
import { copyBarcodeAsPng, downloadBarcodePng, openBarcodePrintView } from '../lib/capacitorUtils'
import { reagentFormSchema, type ReagentFormValues } from '@chemtools/shared/lib/schemas'
import { useToast } from '@chemtools/shared/ui/Toast'
import { ZapIcon, CheckIcon, CopyImgIcon, InfoIcon } from '@chemtools/shared/ui/icons'
...
export function EncodePage() {
  ...
  const { register, handleSubmit, reset, watch, setValue, formState: { errors } } = useForm<ReagentFormValues>({
    ...
    resolver: zodResolver(reagentFormSchema) as any,
      ...
      <Card className="lg:w-[440px] xl:w-[480px] flex flex-col min-h-0 min-w-0">
        ...
        <div className="flex-1 overflow-auto p-5 flex flex-col gap-4">
          <BarcodeSection
            ...
            onPng={() => result && downloadBarcodePng(result.encodedAscii, 'reagent-long.png')}
            onPrint={() => result && openBarcodePrintView(result.encodedAscii, '长码条码打印')}
          />
          ...
          <BarcodeSection
            ...
            onPng={() => result && downloadBarcodePng(result.shortAscii, 'reagent-short.png')}
            onPrint={() => result && openBarcodePrintView(result.shortAscii, '短码条码打印')}
          />
          <div className="flex-1" />
          <div
...
function BarcodeSection({
  title, svg, ascii, onPng, onPrint,
}: {
  title: string; svg: string | null; ascii: string
  onPng: () => void; onPrint: () => void
}) {
  ...
  const handleCopyImage = async () => {
    ...
    setCopying(true)
    try {
      await copyBarcodeAsPng(ascii, title === '短码')
      setCopied(true)
      showToast('复制成功')
    ...
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1.5 items-center">
          ...
          <Button variant="ghost" size="sm" onClick={onPng}>PNG</Button>
          <Button variant="primary" size="sm" onClick={onPrint}>打印</Button>
        </div>
      </div>
      <div
